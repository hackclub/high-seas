...
export async function GET(request, { params }) {
  ...
  const { tickets } = await getPersonTicketBalanceAndTutorialStatutWowThisMethodNameSureIsLongPhew(session.slackId)

  const price = region === 'us' ? item.fields.priceUs : item.fields.priceGlobal
  if (tickets < price) {
    return NextResponse.json({ error: 'Not enough doubloons to buy this item' }, { status: 400 })
  }
  ...
  if (
    ...
    !['Eligible L1', 'Eligible L2'].includes(person.fields.verification_status[0])
  ) {
